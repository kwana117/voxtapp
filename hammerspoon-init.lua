...
-- Remove words duplicated at chunk boundaries caused by the OVERLAP_SECS
-- tail prepended to each enriched chunk. Matches word-by-word (normalised,
-- case- and punctuation-insensitive). Requires ≥2 consecutive matching words
-- to avoid false positives on common short words like "the", "a", etc.
local function dedupeChunks(parts)
    local function norm(w)
        return w:lower():gsub("[%.,%!%?;:%'\"%%]$", ""):gsub("^[%'\"%%]", "")
    end
    ...
    local MAX_WIN = 30  -- ~OVERLAP_SECS * 5 words/sec * 2× safety
    local function overlapLen(prevWords, nextWords)
        local lookback = math.min(#prevWords, MAX_WIN)
        for len = math.min(lookback, #nextWords), 2, -1 do
            local match = true
            for j = 1, len do
                if norm(prevWords[#prevWords - len + j]) ~= norm(nextWords[j]) then
                    match = false
                    break
                end
            end
    ...
    local resultWords = splitWords(parts[1])
    for i = 2, #parts do
        local nextWords = splitWords(parts[i])
        local skip = overlapLen(resultWords, nextWords)
        for j = skip + 1, #nextWords do
            table.insert(resultWords, nextWords[j])
        end
    end
...
maybeFinalize = function()
    ...
    local result = dedupeChunks(parts)
        :gsub("%s+", " ")
        :gsub("^%s+", "")
        :gsub("%s+$", "")
...
local _zionOk, _zionErr = pcall(require, "zion")
if not _zionOk then hs.printf("Zion failed to load: %s", tostring(_zionErr)) end
